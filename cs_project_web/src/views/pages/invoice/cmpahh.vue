      ...
      async exportPdf() {
          ...
          mythis.$root.presentLoading();
            ...
            try {
                ...
                const jobText = `Surat Invoice ${header.job}`;
                let textX;
                if (jobText.length ) {
                ...
                doc.text(':', colonX, 130);
                doc.text(`HARDCORE${header.customer}`, valueX, 130);
                ...
                doc.text(':', colonX, 190);
                doc.text(`HARDCORE${header.trans_date}`, valueX, 190);
  
                doc.text('DUE DATE', labelX, 210);
                doc.text(':', colonX, 210);
                doc.text(`HARDCORE ${header.trans_number}`, valueX, 210);
      ...
      server: {
        ...
        then: (data) =>
          data.results.map((card) => [
            ...
            html(`<span class="pull-left">${card.invoiceno}</span>`),
